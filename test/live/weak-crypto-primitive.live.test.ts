...
const cryptoEvidenceSchema = z.object({
...
class RecordingEvaluator implements Evaluator {
  ...
  async evaluate(request: EvaluationRequest): Promise<Record<string, number>> {
    ...
    for (const [questionId, probability] of Object.entries(answers)) {
      ...
      const evidence = cryptoEvidenceSchema.safeParse(
        candidate?.evidence?.["jev/no-weak-crypto-primitive"],
      );
...
async function lintChangedFile(projectFiles: ProjectFile[], evaluator: Evaluator) {
  ...
  if (!changed) return [];
  const rule = defaultConfig.rules["jev/no-weak-crypto-primitive"];
  expect(rule).toBeDefined();
  if (!rule) return [];
  const config: JevLintConfig = { rules: { "jev/no-weak-crypto-primitive": rule } };
  return analyzeFile(
    {
...
liveDescribe("weak crypto primitive with repository evidence", () => {
  it("separates a password digest from a cache-key digest", async () => {
    ...
    expect(evaluator.probabilities.get("verifyPassword")).toBeGreaterThanOrEqual(0.7);
    expect(evaluator.probabilities.get("cacheKey")).toBeLessThan(0.6);
    expect(smellyJudgments.map(({ ruleId }) => ruleId)).toContain("jev/no-weak-crypto-primitive");
