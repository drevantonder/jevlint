...
const ruleId = "jev/no-deployment-coupled-assumption";

const questionInstructionsSchema = z.object({ inspect: z.string() }).passthrough();
const assumptionEvidenceSchema = z.object({
...
class RecordingEvaluator implements Evaluator {
  ...
  async evaluate(request: EvaluationRequest): Promise<Record<string, number>> {
    ...
    for (const [questionId, probability] of Object.entries(answers)) {
      ...
      const evidence = assumptionEvidenceSchema.safeParse(candidate?.evidence?.[ruleId]);
...
liveDescribe("deployment coupled assumption calibration", () => {
  it("separates the baked-in localhost from the config-read fallback", async () => {
    const smellyEvaluator = new RecordingEvaluator();
    const cleanEvaluator = new RecordingEvaluator();
    const [smellyJudgments, cleanJudgments] = await Promise.all([
      loadProject("deployment-coupled-assumption-positive", [
        "src/client.ts",
        "src/caller.ts",
      ]).then((files) => lintChangedFile(files, smellyEvaluator)),
      loadProject("deployment-coupled-assumption-negative", [
        "src/client.ts",
      ]).then((files) => lintChangedFile(files, cleanEvaluator)),
    ]);

    expect(smellyEvaluator.probabilities.get("fetchOrders")).toBeGreaterThanOrEqual(0.7);
