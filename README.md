...
- Twenty-seven bundled Jev rules
...
The eleven accidental-complexity rules use Oxc for factual candidate discovery and evidence collection, then ask Jev to distinguish a smell from legitimate boundaries, workflows, variation, policy, and dependency constraints. The explicit-effects rules limit judgment to concrete runtime reads and mutable initialization dependencies. They exempt named boundary adapters, providers, lifecycle contracts, and callback-scoped contracts. The scattered-policy rule proves repetition in the current repository; it does not infer historical shotgun change without change-history evidence. The state-modeling rules distinguish correlated lifecycle flags from independent booleans, closed internal states from open strings, and conditionally required payloads from optional metadata or boundary contracts. The four domain-boundary rules separate domain decisions from transport translation, keep persistence-owned records from becoming application or domain contracts, preserve distinct domain identities in project APIs, and keep business policy out of adapters. They preserve legitimate controllers, protocol obligations, domain reconstruction, read models, persistence tooling, ordinary value pairs, protocol representations, and technical adapter decisions. The principles were adapted from Cursor's [Thermo-Nuclear Code Quality Review](https://github.com/cursor/plugins/blob/main/cursor-team-kit/skills/thermo-nuclear-code-quality-review/SKILL.md) and Matt Pocock's [Code Review](https://github.com/mattpocock/skills/blob/main/skills/engineering/code-review/SKILL.md).
