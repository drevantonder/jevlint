...
async function calibrate(name: string, paths: string[]) {
  ...
  const rule = defaultConfig.rules["jev/no-interaction-pinning-test"];
  expect(changed).toBeDefined();
  expect(rule).toBeDefined();
  if (!changed || !rule) return { judgments: [], probability: undefined };
  const config: JevLintConfig = { rules: { "jev/no-interaction-pinning-test": rule } };
...
liveDescribe("interaction pinning test calibration", () => {
  it("separates interaction pinning from outcome assertions", async () => {
    const positive = await calibrate("interaction-positive", [
      "test/calc.test.ts",
      "src/calc.ts",
    ]);
    const negative = await calibrate("interaction-negative", [
      "test/calc.test.ts",
      "src/calc.ts",
    ]);

    expect(positive.probability).toBeGreaterThanOrEqual(0.85);
    expect(positive.judgments.map(({ ruleId }) => ruleId)).toContain("jev/no-interaction-pinning-test");
