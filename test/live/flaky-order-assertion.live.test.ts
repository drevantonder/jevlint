...
async function calibrate(name: string, paths: string[]) {
  ...
  const rule = defaultConfig.rules["jev/no-flaky-order-assertion"];
  expect(changed).toBeDefined();
  expect(rule).toBeDefined();
  if (!changed || !rule) return { judgments: [], probability: undefined };
  const config: JevLintConfig = { rules: { "jev/no-flaky-order-assertion": rule } };
...
liveDescribe("flaky order assertion calibration", () => {
  it("separates unsynchronized order assertions from joined sorted comparisons", async () => {
    const positive = await calibrate("flaky-order-smelly", ["test/scheduler.test.ts"]);
    const negative = await calibrate("flaky-order-negative", ["test/scheduler.test.ts"]);
