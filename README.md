...
- Two hundred ninety-three bundled Jev rules
...
- tests that feed unseeded randomness or live time into their expectations
- domain functions that grab ambient singleton state their tests cannot substitute
- tests that bury their behavior under inline setup mass
- tests that pin module internals rather than observable behavior
- tests that assert an order over concurrent work that nothing synchronizes
...
- changes that close a module dependency cycle across layer boundaries
- domain functions that depend directly on outer-layer modules
- barrels that aggregate unrelated responsibilities behind one surface
- new exports landing in shared utility modules with unrelated responsibilities
- added files that duplicate a responsibility a sibling module already owns
- added functions that duplicate a same-module sibling with only small edits
- exported helpers serving exactly one caller without public re-export
- changes that restate a literal value set another module already owns as a type
- types that duplicate a field shape owned by another module
- changes that add one case sibling code must mirror with nothing forcing the mirror
- changes that widen the exported mutable surface other modules can depend on
- overrides that duplicate base logic with edits instead of reusing it through super
- changes that narrow what existing callers may pass while current call sites break
- calls reaching a deeper module past the intermediate layer that guards it
- loops making one cross-module call per item where a single batch call would serve
- classes serving disjoint caller populations through one shared surface
- shipped files reaching development-only dependencies at runtime
- calls assuming an order across a module boundary that nothing enforces
- module surfaces split between callback and return completion styles
- exported entries reachable both as callbacks and as direct calls against mutated module state
- exported signatures exposing infrastructure or library implementation types
...
- exported functions retained although nothing calls, imports, or re-exports them
- comment blocks holding disabled implementations instead of documentation
- compat-named layers retained although their successor carries the callers
- predicate-shaped names that return non-boolean values
- comments that confess confusion without recording what was verified
- suppressions that disable a check without recording why
- new consumers reaching into the shared kernel from previously-unconnected contexts
- new dependencies running against the established direction between two areas
- runtime modules newly depending on another context's test or fixture sources
- new wraps of an external package another context already gateways
- new external service hosts called outside the established client wrapper
