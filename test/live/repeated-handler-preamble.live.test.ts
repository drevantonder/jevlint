...
const RULE = "jev/no-repeated-handler-preamble";
...
liveDescribe("repeated handler preamble calibration", () => {
  it("separates shared ceremony from distinct per-site handling", async () => {
    const positive = await review("repeated-preamble-positive", [
      "src/users.ts",
      "src/telemetry.ts",
    ]);
    const negative = await review("repeated-preamble-negative", ["src/users.ts"]);
