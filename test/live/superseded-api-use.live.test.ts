...
async function calibrate(name: string, paths: string[]) {
  ...
  const rule = defaultConfig.rules["jev/no-superseded-api-use"];
  expect(changed).toBeDefined();
  expect(rule).toBeDefined();
  if (!changed || !rule) return { judgments: [], probability: undefined };
  const config: JevLintConfig = { rules: { "jev/no-superseded-api-use": rule } };
...
liveDescribe("superseded api use calibration", () => {
  it("separates superseded calls from successor calls", async () => {
    const positive = await calibrate("superseded-positive", [
      "src/encrypt.ts",
      "src/cipher.ts",
      "src/modern.ts",
    ]);
    const negative = await calibrate("superseded-negative", [
      "src/encrypt.ts",
      "src/cipher.ts",
    ]);

    expect(positive.probability).toBeGreaterThanOrEqual(0.85);
    expect(positive.judgments.map(({ ruleId }) => ruleId)).toContain("jev/no-superseded-api-use");
