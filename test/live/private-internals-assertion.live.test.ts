...
async function calibrate(name: string, paths: string[]) {
  ...
  const rule = defaultConfig.rules["jev/no-private-internals-assertion"];
  expect(changed).toBeDefined();
  expect(rule).toBeDefined();
  if (!changed || !rule) return { judgments: [], probability: undefined };
  const config: JevLintConfig = { rules: { "jev/no-private-internals-assertion": rule } };
...
liveDescribe("private internals assertion calibration", () => {
  it("separates internals-pinning tests from public-behavior tests", async () => {
    const positive = await calibrate("private-internals-smelly", [
      "test/store.test.ts",
      "src/index.ts",
      "src/internal/store.ts",
    ]);
    const negative = await calibrate("private-internals-negative", [
      "test/store.test.ts",
      "src/index.ts",
      "src/internal/store.ts",
    ]);
