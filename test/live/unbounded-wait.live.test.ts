...
async function calibrate(name: string, paths: string[]) {
  ...
  const rule = defaultConfig.rules["jev/no-unbounded-wait"];
  expect(changed).toBeDefined();
  expect(rule).toBeDefined();
  if (!changed || !rule) return { judgments: [], probability: undefined };
  const config: JevLintConfig = { rules: { "jev/no-unbounded-wait": rule } };
...
liveDescribe("unbounded wait calibration", () => {
  it("separates bare remote waits from bounded fetches and wrapper defaults", async () => {
    const positive = await calibrate("unbounded-wait-positive", [
      "src/fetch-user.ts",
      "src/handler.ts",
    ]);
    const negative = await calibrate("unbounded-wait-negative", ["src/fetch-user.ts"]);
    const ambiguous = await calibrate("unbounded-wait-ambiguous", [
      "src/fetch-user.ts",
      "src/http.ts",
    ]);

    expect(positive.probability).toBeGreaterThanOrEqual(0.85);
    expect(positive.judgments.map(({ ruleId }) => ruleId)).toEqual(["jev/no-unbounded-wait"]);
