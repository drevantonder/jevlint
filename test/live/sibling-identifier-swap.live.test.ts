...
const ruleId = "jev/no-sibling-identifier-swap";
...
liveDescribe("sibling identifier swap calibration", () => {
  it("separates the twice-validated grantType from the fully referenced pair", async () => {
    const positive = await calibrate("sibling-identifier-swap-positive", [
      "src/token.ts",
      "src/caller.ts",
    ]);
    const negative = await calibrate("sibling-identifier-swap-negative", ["src/token.ts"]);
