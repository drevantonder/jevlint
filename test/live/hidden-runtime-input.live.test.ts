...
async function lint(projectFiles: ProjectFile[], evaluator: Evaluator) {
  ...
  const rule = defaultConfig.rules["jev/no-hidden-runtime-input"];
  expect(changed).toBeDefined();
  expect(rule).toBeDefined();
  if (!changed || !rule) return [];
  const config: JevLintConfig = { rules: { "jev/no-hidden-runtime-input": rule } };
  return analyzeFile({
...
liveDescribe("hidden runtime input calibration", () => {
  it("separates hidden policy input from explicit input and runtime adapters", async () => {
    const evaluator = new RecordingEvaluator();
    const [positive, negative, exception, ambiguous] = await Promise.all([
      project("hidden-runtime-input-positive", ["src/quote-shipping.ts", "src/checkout.ts"]),
      project("hidden-runtime-input-negative", ["src/quote-shipping-with-policy.ts", "src/checkout.ts"]),
    ...
    expect(evaluator.probabilities.get("src/quote-shipping.ts")).toBeGreaterThanOrEqual(0.85);
