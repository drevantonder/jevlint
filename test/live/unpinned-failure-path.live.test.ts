...
liveDescribe("unpinned failure path calibration", () => {
  it("separates an untested error mapping from a pinned one", async () => {
    const positive = await calibrate("failure-positive", [
      "src/charge.ts",
      "src/gateway.ts",
      "test/other.test.ts",
    ]);
    const negative = await calibrate("failure-negative", [
      "src/charge.ts",
      "src/gateway.ts",
      "test/charge.test.ts",
    ]);
    ...
    expect(positive.judgments.map(({ ruleId }) => ruleId)).toContain("jev/no-unpinned-failure-path");
