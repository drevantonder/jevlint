...
const ruleId = "jev/no-missing-shutdown-drain";

const questionInstructionsSchema = z.object({ inspect: z.string() }).passthrough();
const drainEvidenceSchema = z.object({
...
class RecordingEvaluator implements Evaluator {
  ...
  async evaluate(request: EvaluationRequest): Promise<Record<string, number>> {
    ...
    for (const [questionId, probability] of Object.entries(answers)) {
      ...
      const evidence = drainEvidenceSchema.safeParse(candidate?.evidence?.[ruleId]);
...
liveDescribe("missing shutdown drain calibration", () => {
  it("separates the bare listen from the SIGTERM-drained bootstrap", async () => {
    const smellyEvaluator = new RecordingEvaluator();
    const cleanEvaluator = new RecordingEvaluator();
    const [smellyJudgments, cleanJudgments] = await Promise.all([
      loadProject("missing-shutdown-drain-positive", [
        "src/server.ts",
        "src/app.ts",
        "k8s/deployment.yaml",
      ]).then((files) => lintChangedFile(files, smellyEvaluator)),
      loadProject("missing-shutdown-drain-negative", [
        "src/server.ts",
        "src/app.ts",
      ]).then((files) => lintChangedFile(files, cleanEvaluator)),
    ]);

    expect(smellyEvaluator.probabilities.get("startServer")).toBeGreaterThanOrEqual(0.7);
