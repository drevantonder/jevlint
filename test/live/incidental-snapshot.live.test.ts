...
async function calibrate(name: string, paths: string[]) {
  ...
  const rule = defaultConfig.rules["jev/no-incidental-snapshot"];
  expect(changed).toBeDefined();
  expect(rule).toBeDefined();
  if (!changed || !rule) return { judgments: [], probability: undefined };
  const config: JevLintConfig = { rules: { "jev/no-incidental-snapshot": rule } };
...
liveDescribe("incidental snapshot calibration", () => {
  it("separates an incidental tree snapshot from a contractual one", async () => {
    const positive = await calibrate("snapshot-positive", [
      "test/page.test.ts",
      "src/page.ts",
    ]);
    const negative = await calibrate("snapshot-negative", [
      "test/order.test.ts",
      "src/order.ts",
    ]);

    expect(positive.probability).toBeGreaterThanOrEqual(0.85);
    expect(positive.judgments.map(({ ruleId }) => ruleId)).toContain("jev/no-incidental-snapshot");
