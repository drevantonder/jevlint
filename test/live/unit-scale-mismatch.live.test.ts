...
const RULE = "jev/no-unit-scale-mismatch";
...
const SMELLY: ProjectFile = {
  ...
  source: "export function scheduleRetry(timeoutSec: number) {\n"
...
liveDescribe("unit scale mismatch with structural evidence", () => {
  it("flags ad-hoc scale factors while scale-free functions abstain", async () => {
    ...
    expect(evaluator.probabilities.get("scheduleRetry")).toBeGreaterThanOrEqual(0.7);
  });
});
