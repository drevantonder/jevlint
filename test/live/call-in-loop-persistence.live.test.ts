...
async function calibrate(name: string, paths: string[]) {
  ...
  const rule = defaultConfig.rules["jev/no-call-in-loop-persistence"];
  expect(changed).toBeDefined();
  expect(rule).toBeDefined();
  if (!changed || !rule) return { judgments: [], probability: undefined };
  const config: JevLintConfig = { rules: { "jev/no-call-in-loop-persistence": rule } };
...
liveDescribe("call in loop persistence calibration", () => {
  it("separates per-item persistence round-trips from local loop work", async () => {
    const [positive, negative] = await Promise.all([
      calibrate("call-in-loop-persistence-positive", [
        "src/order-service.ts",
        "src/order-repo.ts",
      ]),
      calibrate("call-in-loop-persistence-negative", [
        "src/sum.ts",
      ]),
    ]);
    ...
    expect(positive.judgments.map(({ ruleId }) => ruleId)).toEqual(["jev/no-call-in-loop-persistence"]);
