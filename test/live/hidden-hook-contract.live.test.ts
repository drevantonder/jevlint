...
const RULE_ID = "jev/no-hidden-hook-contract";
...
async function lint(projectFiles: ProjectFile[], evaluator: Evaluator) {
  const changed = projectFiles.find(({ filePath }) => filePath === "src/user.ts");
...
liveDescribe("hidden hook contract with structural evidence", () => {
  it("flags a plain-named hook caller but keeps the declared custom hook", async () => {
    ...
    expect(smellyEvaluator.probabilities.get("src/user.ts")).toBeGreaterThanOrEqual(0.85);
