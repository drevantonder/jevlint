...
const ruleId = "jev/no-inappropriate-intimacy";
...
liveDescribe("inappropriate intimacy calibration", () => {
  it("flags interior digging but keeps advertised use", async () => {
    const evaluator = new RecordingEvaluator();
    const projectFiles = await project("inappropriate-intimacy-smelly", [
      "src/job-queue.ts",
      "src/drain.ts",
      "src/standalone.ts",
    ]);

    const judgments = await lintChangedFile(projectFiles, "src/drain.ts", evaluator);

    expect(evaluator.probabilities.get("drainStale")).toBeGreaterThanOrEqual(0.7);
    expect(evaluator.probabilities.get("schedule")).toBeLessThan(0.6);
