...
const ruleId = "jev/no-pre-gate-side-effect";
...
liveDescribe("pre-gate side effect calibration", () => {
  it("separates the pre-limit cache write from the gated write", async () => {
    const positive = await calibrate("pre-gate-side-effect-positive", [
      "src/device.ts",
      "src/cache.ts",
      "src/caller.ts",
    ]);
    const negative = await calibrate("pre-gate-side-effect-negative", [
      "src/device.ts",
      "src/cache.ts",
    ]);
