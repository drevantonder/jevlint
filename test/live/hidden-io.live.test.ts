...
liveDescribe("hidden I/O calibration", () => {
  it("separates concealed network cost from local work, disclosed fetches, and uncertain awaits", async () => {
    const positive = await calibrate("hidden-io-positive", [
      "src/calculate-shipping-quote.ts",
      "src/shipping-gateway.ts",
      "src/checkout.ts",
    ]);
    const negative = await calibrate("hidden-io-negative", ["src/calculate-shipping-quote.ts"]);
    const exception = await calibrate("hidden-io-exception", ["src/fetch-shipping-quote.ts"]);
