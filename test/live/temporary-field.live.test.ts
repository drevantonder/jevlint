...
const ruleId = "jev/no-temporary-field";
...
liveDescribe("temporary field calibration", () => {
  it("flags partial-lifetime state but keeps construction-time state", async () => {
    const evaluator = new RecordingEvaluator();
    const projectFiles = await project("temporary-field-smelly", [
      "src/document.ts",
      "src/preview.ts",
      "src/strict.ts",
    ]);

    const [documentJudgments, strictJudgments] = await Promise.all([
      lintChangedFile(projectFiles, "src/document.ts", evaluator),
      lintChangedFile(projectFiles, "src/strict.ts", evaluator),
    ]);

    expect(evaluator.probabilities.get("Document")).toBeGreaterThanOrEqual(0.7);
    expect(strictJudgments).toHaveLength(0);
    expect(documentJudgments.map(({ ruleId: judged }) => judged)).toContain(ruleId);
  });
});
