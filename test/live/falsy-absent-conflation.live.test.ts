...
async function calibrate(name: string, paths: string[]) {
  ...
  const rule = defaultConfig.rules["jev/no-falsy-absent-conflation"];
  expect(changed).toBeDefined();
  expect(rule).toBeDefined();
  if (!changed || !rule) return { judgments: [], probability: undefined };
  const config: JevLintConfig = { rules: { "jev/no-falsy-absent-conflation": rule } };
...
liveDescribe("falsy absent conflation calibration", () => {
  it("separates falsy-conflating checks from checks on domains without valid falsy values", async () => {
    const [positive, negative] = await Promise.all([
      calibrate("falsy-absent-conflation-positive", [
        "src/sampling.ts",
        "src/schema.ts",
        "src/app.ts"
      ]),
      calibrate("falsy-absent-conflation-negative", [
        "src/sampling.ts",
        "src/schema.ts",
        "src/app.ts"
      ]),
    ...
    expect(positive.judgments.map(({ ruleId }) => ruleId)).toEqual(["jev/no-falsy-absent-conflation"]);
