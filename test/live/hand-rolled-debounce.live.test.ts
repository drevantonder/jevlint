...
async function review(projectFiles: ProjectFile[]) {
  const changed = projectFiles.find((file) => file.filePath.includes("search"));
...
liveDescribe("hand rolled debounce calibration", () => {
  it("scores a full-option wrapper beside lodash.debounce above a dep-free project", async () => {
    const smelly = await review(await loadProject("hand-rolled-debounce-positive", [
      "package.json",
      "src/search.ts",
      "src/input.ts",
    ]));
    const clean = await review([
      { filePath: "package.json", source: JSON.stringify({ dependencies: {} }) },
      {
        ...
        source: "export function later(fn: () => void): void {\n"
          + "  setTimeout(fn, 100);\n"
          + "}\n",
      },
    ]);
