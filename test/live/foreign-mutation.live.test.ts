...
const STORE: ProjectFile = {
  filePath: "src/store.ts",
  source: "export const cache: Record<string, string> = {};\n",
};
const SMELLY: ProjectFile = {
  filePath: "src/policy.ts",
  source: "import { cache } from \"./store.js\";\n"
...
const CLEAN: ProjectFile = {
  filePath: "src/policy.ts",
  source: "export function mergedCache(entries: Record<string, string>): Record<string, string> {\n"
    + "  return { ...entries };\n"
    + "}\n",
};
...
async function lint(projectFiles: ProjectFile[], evaluator: Evaluator) {
  ...
  const rule = defaultConfig.rules["jev/no-foreign-mutation"];
  expect(changed).toBeDefined();
  expect(rule).toBeDefined();
  if (!changed || !rule) return [];
  const config: JevLintConfig = { rules: { "jev/no-foreign-mutation": rule } };
  return analyzeFile({
...
liveDescribe("foreign mutation with structural evidence", () => {
  it("flags writes to another module's state but keeps local copies", async () => {
    const smellyEvaluator = new RecordingEvaluator();
    const cleanEvaluator = new RecordingEvaluator();
    const [smellyJudgments, cleanJudgments] = await Promise.all([
      lint([SMELLY, STORE], smellyEvaluator),
