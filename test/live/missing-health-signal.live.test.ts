...
const ruleId = "jev/no-missing-health-signal";
...
class RecordingEvaluator implements Evaluator {
  ...
  async evaluate(request: EvaluationRequest): Promise<Record<string, number>> {
    ...
    for (const [questionId, probability] of Object.entries(answers)) {
      ...
      const evidence = healthEvidenceSchema.safeParse(candidate?.evidence?.[ruleId]);
...
liveDescribe("missing health signal calibration", () => {
  it("separates the probeless service from the one serving /healthz", async () => {
    const smellyEvaluator = new RecordingEvaluator();
    const cleanEvaluator = new RecordingEvaluator();
    const [smellyJudgments, cleanJudgments] = await Promise.all([
      loadProject("missing-health-signal-positive", [
        "src/service.ts",
        "src/app.ts",
        "src/users.ts",
        "k8s/deployment.yaml",
      ]).then((files) => lintChangedFile(files, smellyEvaluator)),
      loadProject("missing-health-signal-negative", [
        "src/service.ts",
        "src/app.ts",
        "src/users.ts",
    ...
    expect(smellyEvaluator.probabilities.get("startService")).toBeGreaterThanOrEqual(0.7);
