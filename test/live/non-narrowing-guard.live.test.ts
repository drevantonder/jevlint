...
const RULE = "jev/no-non-narrowing-guard";
...
liveDescribe("non narrowing guard calibration", () => {
  it("scores a redundant re-check while a lone guard abstains", async () => {
    const positive = await review("non-narrowing-positive", ["src/describe.ts"]);
    const negative = await review("non-narrowing-negative", ["src/greet.ts"]);
