...
async function calibrate(name: string, paths: string[]) {
  ...
  const rule = defaultConfig.rules["jev/no-concurrent-shared-mutation"];
  expect(changed).toBeDefined();
  expect(rule).toBeDefined();
  if (!changed || !rule) return { judgments: [], probability: undefined };
  const config: JevLintConfig = { rules: { "jev/no-concurrent-shared-mutation": rule } };
...
liveDescribe("concurrent shared mutation calibration", () => {
  it("separates uncoordinated concurrent writes from sequential mutation", async () => {
    const [positive, negative] = await Promise.all([
      calibrate("concurrent-shared-mutation-positive", [
        "src/collect.ts",
        "src/client.ts",
      ]),
      calibrate("concurrent-shared-mutation-negative", [
        "src/collect.ts",
        "src/client.ts",
      ]),
    ]);
    ...
    expect(positive.judgments.map(({ ruleId }) => ruleId)).toEqual(["jev/no-concurrent-shared-mutation"]);
