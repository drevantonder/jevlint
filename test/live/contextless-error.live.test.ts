...
async function calibrate(name: string, paths: string[]) {
  ...
  const rule = defaultConfig.rules["jev/no-contextless-error"];
  expect(changed).toBeDefined();
  expect(rule).toBeDefined();
  if (!changed || !rule) return { judgments: [], probability: undefined };
  const config: JevLintConfig = { rules: { "jev/no-contextless-error": rule } };
...
liveDescribe("contextless error calibration", () => {
  it("separates errors born empty from cause-linked failures", async () => {
    const positive = await calibrate("contextless-error-positive", [
      "src/save-record.ts",
      "src/store.ts",
      "src/service.ts",
    ]);
    const negative = await calibrate("contextless-error-negative", [
      "src/save-record.ts",
      "src/store.ts",
    ]);

    expect(positive.probability).toBeGreaterThanOrEqual(0.85);
    expect(positive.judgments.map(({ ruleId }) => ruleId)).toEqual(["jev/no-contextless-error"]);
