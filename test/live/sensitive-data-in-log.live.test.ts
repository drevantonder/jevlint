...
const SMELLY: ProjectFile = {
  ...
  source: "import { logger } from \"./log.js\";\n"
...
async function lint(projectFiles: ProjectFile[], evaluator: Evaluator) {
  ...
  const rule = defaultConfig.rules["jev/no-sensitive-data-in-log"];
  expect(changed).toBeDefined();
  expect(rule).toBeDefined();
  if (!changed || !rule) return [];
  const config: JevLintConfig = { rules: { "jev/no-sensitive-data-in-log": rule } };
  return analyzeFile({
...
liveDescribe("sensitive data in log with structural evidence", () => {
  it("flags a logged record carrying secrets but keeps an explicit field pick", async () => {
