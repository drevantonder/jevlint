...
async function calibrate(name: string, paths: string[]) {
  ...
  const rule = defaultConfig.rules["jev/no-unbounded-accumulation"];
  expect(changed).toBeDefined();
  expect(rule).toBeDefined();
  if (!changed || !rule) return { judgments: [], probability: undefined };
  const config: JevLintConfig = { rules: { "jev/no-unbounded-accumulation": rule } };
...
liveDescribe("unbounded accumulation calibration", () => {
  it("separates module-lived growth without eviction from request-scoped use", async () => {
    const [positive, negative] = await Promise.all([
      calibrate("unbounded-accumulation-positive", [
        "src/seen-urls.ts",
        "src/handler.ts",
      ]),
      calibrate("unbounded-accumulation-negative", [
        "src/build-report.ts",
      ]),
    ]);
    ...
    expect(positive.judgments.map(({ ruleId }) => ruleId)).toEqual(["jev/no-unbounded-accumulation"]);
