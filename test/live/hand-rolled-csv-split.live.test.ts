...
async function review(projectFiles: ProjectFile[]) {
  const changed = projectFiles.find((file) => file.filePath.includes("csv"));
...
liveDescribe("hand rolled csv split calibration", () => {
  it("scores naive splitting beside papaparse above a dep-free project", async () => {
    const smelly = await review(await loadProject("hand-rolled-csv-split-positive", [
      "package.json",
      "src/csv.ts",
      "src/importer.ts",
    ]));
    const clean = await review([
      ...
      {
        filePath: "src/csv.ts",
        source: "export function lines(input: string): string[] {\n"
          + "  return input.split(\"\\n\").map((line) => line.trim());\n"
          + "}\n",
      },
    ]);
