...
const ruleId = "jev/no-hardcoded-config-shadow";
...
liveDescribe("hardcoded config shadow calibration", () => {
  it("separates the restated limit from the config-reading call site", async () => {
    const positive = await calibrate("hardcoded-config-shadow-positive", [
      "src/upload.ts",
      "src/settings.ts",
      "src/caller.ts",
    ]);
    const negative = await calibrate("hardcoded-config-shadow-negative", [
      "src/upload.ts",
      "src/settings.ts",
    ]);
