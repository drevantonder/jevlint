...
async function calibrate(name: string, paths: string[]) {
  ...
  const rule = defaultConfig.rules["jev/no-unverified-mock-contract"];
  expect(changed).toBeDefined();
  expect(rule).toBeDefined();
  if (!changed || !rule) return { judgments: [], probability: undefined };
  const config: JevLintConfig = { rules: { "jev/no-unverified-mock-contract": rule } };
...
liveDescribe("unverified mock contract calibration", () => {
  it("separates contract-divergent stubs from faithful boundary stubs", async () => {
    const positive = await calibrate("mock-contract-positive", [
      "test/client.test.ts",
      "src/api/client.ts",
    ]);
    const negative = await calibrate("mock-contract-negative", [
      "test/clock.test.ts",
      "src/clock.ts",
    ]);
    const exception = await calibrate("mock-contract-external", [
      "test/remote.test.ts",
    ]);

    expect(positive.probability).toBeGreaterThanOrEqual(0.85);
    expect(positive.judgments.map(({ ruleId }) => ruleId)).toContain("jev/no-unverified-mock-contract");
