...
liveDescribe("unexplained suppression calibration", () => {
  it("judges the bare suppression while the explained one abstains", async () => {
    const bare = await review("unexplained-suppression-positive", ["src/read.ts"]);
    const explained = await review("unexplained-suppression-explained", ["src/announce.ts"]);

    expect(bare.judgments.map(({ ruleId }) => ruleId)).toContain(RULE);
    expect(explained.judgments.map(({ ruleId }) => ruleId)).not.toContain(RULE);
  });
});
