...
liveDescribe("lossy sentinel return calibration", () => {
  it("separates caller-relevant information loss from explicit results and intentional collapse", async () => {
    const positive = await calibrate("lossy-sentinel-return-positive", [
      "src/choose-delivery-slot.ts",
      "src/checkout.ts",
    ]);
    const negative = await calibrate("lossy-sentinel-return-negative", ["src/choose-delivery-slot.ts"]);
    const exception = await calibrate("lossy-sentinel-return-exception", ["src/find-active-session.ts"]);
