...
liveDescribe("swallowed error calibration", () => {
  it("separates hidden failure from propagation, best effort, and uncertain fallback", async () => {
    const [positive, negative, legitimate, ambiguous] = await Promise.all([
      calibrate("swallowed-error-positive", [
        "src/publish-invoice.ts",
        "src/services.ts",
        "src/billing.ts",
      ]),
      calibrate("swallowed-error-negative", [
        "src/store-receipt.ts",
        "src/receipt-store.ts",
        "src/checkout.ts",
      ]),
      calibrate("swallowed-error-legitimate", [
        "src/delete-account.ts",
        "src/services.ts",
        "src/settings.ts",
      ]),
      calibrate("swallowed-error-ambiguous", [
        "src/load-profile.ts",
        "src/services.ts",
        "src/profile-page.ts",
      ]),
