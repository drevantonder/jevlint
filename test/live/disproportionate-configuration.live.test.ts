...
async function lint(projectFiles: ProjectFile[]) {
  ...
  const rule = defaultConfig.rules["jev/no-disproportionate-configuration"];
  expect(changed).toBeDefined();
  expect(rule).toBeDefined();
  if (!changed || !rule) return [];
  const config: JevLintConfig = { rules: { "jev/no-disproportionate-configuration": rule } };
  return analyzeFile({
...
liveDescribe("disproportionate configuration with repository evidence", () => {
  it("flags an option surface far beyond observed use but keeps exercised request options", async () => {
    const [smelly, real] = await Promise.all([
      project("disproportionate-config-smelly", [
