...
const ruleId = "jev/no-silent-queue-drop";
...
class RecordingEvaluator implements Evaluator {
  ...
  async evaluate(request: EvaluationRequest): Promise<Record<string, number>> {
    ...
    for (const [questionId, probability] of Object.entries(answers)) {
      ...
      const evidence = queueEvidenceSchema.safeParse(candidate?.evidence?.[ruleId]);
...
liveDescribe("silent queue drop calibration", () => {
  it("separates the unchecked publish from the policy-guarded one", async () => {
    const smellyEvaluator = new RecordingEvaluator();
    const cleanEvaluator = new RecordingEvaluator();
    const [smellyJudgments, cleanJudgments] = await Promise.all([
      loadProject("silent-queue-drop-positive", [
        "src/publish.ts",
        "src/queue.ts",
        "src/route.ts",
      ]).then((files) => lintChangedFile(files, smellyEvaluator)),
      loadProject("silent-queue-drop-negative", [
        "src/publish.ts",
        "src/queue.ts",
      ]).then((files) => lintChangedFile(files, cleanEvaluator)),
    ]);

    expect(smellyEvaluator.probabilities.get("handleRequest")).toBeGreaterThanOrEqual(0.7);
