...
const ruleId = "jev/no-inverted-authorization-predicate";
...
liveDescribe("inverted authorization predicate calibration", () => {
  it("separates the conjunctive admin-owner check from the disjunctive one", async () => {
    const positive = await calibrate("inverted-authorization-predicate-positive", [
      "src/team.ts",
      "src/caller.ts",
    ]);
    const negative = await calibrate("inverted-authorization-predicate-negative", ["src/team.ts"]);
