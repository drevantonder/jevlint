...
async function lint(projectFiles: ProjectFile[]) {
  ...
  const rule = defaultConfig.rules["jev/no-needless-abstraction"];
  expect(changed).toBeDefined();
  expect(rule).toBeDefined();
  if (!changed || !rule) return [];
  const config: JevLintConfig = { rules: { "jev/no-needless-abstraction": rule } };
  return analyzeFile({
...
liveDescribe("needless abstraction with repository evidence", () => {
  it("flags a one-use ceremony layer but keeps a real dependency port", async () => {
    const [smelly, real] = await Promise.all([
      project("needless-abstraction-smelly", [
        "src/user-name-formatter.ts",
        "src/profile.ts",
      ]),
      project("needless-abstraction-real", [
