...
async function calibrate(name: string, paths: string[]) {
  ...
  const rule = defaultConfig.rules["jev/no-quarantined-test-coverage"];
  expect(changed).toBeDefined();
  expect(rule).toBeDefined();
  if (!changed || !rule) return { judgments: [], probability: undefined };
  const config: JevLintConfig = { rules: { "jev/no-quarantined-test-coverage": rule } };
...
liveDescribe("quarantined test coverage calibration", () => {
  it("separates a lone skipped test from a duplicated one", async () => {
    const positive = await calibrate("quarantine-positive", [
      "test/calc.test.ts",
      "src/calc.ts",
    ]);
    const negative = await calibrate("quarantine-negative", [
      "test/calc.test.ts",
      "test/totals.test.ts",
      "src/calc.ts",
    ]);
    ...
    expect(positive.judgments.map(({ ruleId }) => ruleId)).toContain("jev/no-quarantined-test-coverage");
