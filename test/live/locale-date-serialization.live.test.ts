...
const RULE = "jev/no-locale-date-serialization";
...
const SMELLY: ProjectFile = {
  filePath: "src/events-smelly.ts",
  source: "export function persistEvent(when: Date) {\n"
    + "  return db.events.insert({ at: when.toLocaleString() });\n"
    + "}\n",
};
const CLEAN: ProjectFile = {
  filePath: "src/events-clean.ts",
  source: "export function persistEventIso(when: Date) {\n"
...
liveDescribe("locale date serialization with structural evidence", () => {
  it("flags locale strings at the boundary while ISO persistence abstains", async () => {
    ...
    expect(evaluator.probabilities.get("persistEvent")).toBeGreaterThanOrEqual(0.7);
  });
});
